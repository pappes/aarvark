...
class MyHtml {
  ...
  static String setUriParameters(String originalUrl,
    ...
    Uri x = Uri.parse(
        finalUrl.trim()); // Uri.parse() does not work with leading spaces
  ...
  static String? removeUrlRedirect(String originalUrl) {
    ...
    final Map<String, String> params = Uri.parse(originalUrl).queryParameters;
    for (String param in params.values) {
  ...
  static Element createElementFromHTML(String htmlFragment) {
    log.info('Function : createElementFromHTML, '
  ...
  static void removeEventHandler(Element e) {
    ...
    if (e.nodeName!.toLowerCase() != 'script') e.replaceWith(e.clone(true));
...
*/
  ...
  static int _compareElementArea(dynamic a, dynamic b) {
    assert(a is Element);
    assert(b is Element);
    log.info('Function : _compareElementArea, Parameters : {[a,$a][b,$b]}');
    log.fine('a.width=${a.clientWidth} a.height=${a.clientHeight} '
        'b.width=${b.clientWidth} b.height=${b.clientHeight}');
    int comparison;
    if (a.clientWidth * a.clientHeight < b.clientWidth * b.clientHeight)
      comparison = -1;
    else if (a.clientWidth * a.clientHeight == b.clientWidth * b.clientHeight)
      comparison = 0;
    else
      comparison = 1;
